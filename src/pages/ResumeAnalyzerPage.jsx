...
export default function ResumeAnalyzerPage() {
  const [matchMode, setMatchMode] = useState("profile") // "profile" | "text" | "tailor"
  ...
  // Load history list when a job selection changes in Tailoring tab
  useEffect(() => {
    if (matchMode === "tailor" && selectedJobId) {
      const fetchHistory = async () => {
        try {
          const data = await api.getTailoredHistories(selectedJobId)
          setTailorHistory(data || [])
        } catch (err) {
          console.error("Failed to load tailoring histories:", err)
        }
      }
      fetchHistory()
    }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 transition-colors duration-300">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-550 dark:text-slate-400 max-w-2xl mx-auto">Evaluate profile compatibility or automatically optimize and download tailored resumes for active jobs.</p>
      </div>
        ...
        <div className="lg:col-span-3">
          <Card className="p-6 min-h-[500px] border dark:border-slate-800">
            <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-6 flex items-center gap-2 tracking-tight">
              📊 {matchMode === "tailor" ? "Tailored Resume Versions" : "Match Insights Report"}
            </h2>
                      ...
                      <div key={version._id} className={`border dark:border-slate-850 rounded-2xl p-5 space-y-5 ${idx === 0 ? "bg-indigo-50/10 dark:bg-slate-900/40 border-indigo-150" : "bg-white"}`}>
                        ...
                        {version.missingSkills?.length > 0 && (
                          <div className="space-y-1.5">
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Remaining Gaps (Skills to develop)</span>
                        ...
                        {version.suggestions?.length > 0 && (
                          <div className="space-y-1.5">
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Optimization Bullet Points Feedback</span>
                            <ul className="space-y-1 text-[11px] text-gray-650 dark:text-slate-350 list-disc list-inside leading-normal">
              ...
              <div>
                ...
                {activeData && (
                  <div className="space-y-8 animate-fadeIn">
                    ...
                    {/* Strengths/Weaknesses/Suggestions (Master profile mode only) */}
                    {matchMode === "profile" && (
                        ...
                        <div className="space-y-2.5">
                          ...
                          <ul className="space-y-2">
                            {activeData.strengths && activeData.strengths.map((str, idx) => (
                              <li key={idx} className="flex gap-2 text-xs text-gray-650 dark:text-slate-350 leading-relaxed items-start">
